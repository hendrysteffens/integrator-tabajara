...
object ExtractQueryData {
    fun extractDataQuery(entityName: String): ArrayList<FieldQuery> {
        ...
        val query = File(props.getProperty("integrator.queries") + "/g5-$entityName-query.sql").bufferedReader().use { it.readText() }
    ...
    fun extractDataSyncQuery(entityName: String): ArrayList<FieldQuery> {
        val props = Properties()
        props.load(FileInputStream(App().getResouce()))
        val syncQuery = File(props.getProperty("integrator.queries") + "/g5-sync-$entityName-query.sql").bufferedReader().use { it.readText() }
